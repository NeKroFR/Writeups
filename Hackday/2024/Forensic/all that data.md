# All that data??? - 100

We have two files `access.log` and `error.log`.
Also the flag format is: `HACKDAY{attacker_ip:attack_method:plugin_installed}`
The most striking thing about `access.log` is the end of the logs:
```
10.0.3.250 - - [04/May/2023:10:41:06 +0200] "GET /wp-content/plugins/wp_webshell/wp_webshell.php?action=exec&cmd=python3+-c+%22import%20zlib%20;%20import%20base64%20;%20exec(zlib.decompress(base64.b64decode(%%27)))%22 HTTP/1.1" 200 6288 "-" "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/112.0"
```
So here we have the ip: `10.0.3.250`.
We can also see that the attacker is using the plugin `wp_webshell` and after running
```
>> cat access.log | grep wp_webshell  
```
we can see:
```
10.0.3.250 - - [04/May/2023:10:38:50 +0200] "GET /wp-admin/plugins.php?action=activate&plugin=wp_webshell%2Fwp_webshell.php&_wpnonce=83aeee5a35 HTTP/1.1"
```
So the plugin is: `wp_webshell`

Also looking for the logs we can see this pattern:
```
...
"GET /amember HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /analog HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /analyse HTTP/1.1" 404 452 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /analysis HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /analytics HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /and HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /android HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /announce HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /announcement HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /announcements HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /annuaire HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /annual HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /anon HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /anon_ftp HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /anonymous HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /ansi HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /answer HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
"GET /answers HTTP/1.1" 404 433 "-" "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)"
...
```
So we can assume that the attack_method is: `bruteforce`
