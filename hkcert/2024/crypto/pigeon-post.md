...
The challenge sets up a basic Diffie-Hellman (DH) key exchange communication between two users using the RFC-3526 standard.
The goal of the challenge is to perform a basic Man-in-the-Middle (MitM) to retrieve the flag.
...
First here a little recap on how does the protocol works.

![alt-text](https://i.imgur.com/00TB7KB_d.webp?maxwidth=760&fidelity=grand)


Instead of letting Alice and Byron exchange keys directly, we generate our own private key and send our public key to both Alice and Byron.


![alt-text](https://i.imgur.com/WExDffI.png)


Then we just need to follow the communication:
...
r = remote("c24a-pigeon-1.hkcert24.pwnable.hk", 1337, ssl=True)

j = receive(r)
alice_public = j["public_key"]
attacker_private = 42 # can be anything
attacker_public = pow(G, attacker_private, P)
send(r, 'byron', {"type": "receive_handshake", "public_key": attacker_public})

j = receive(r)
byron_public = j["public_key"]
send(r, 'alice', {"type": "finish_handshake", "public_key": attacker_public})

session_key_alice = derive_session_key(alice_public, attacker_private)
session_key_byron = derive_session_key(byron_public, attacker_private)

j = receive(r)
ciphertext = bytes.fromhex(j["ciphertext"])
message = decrypt_message(session_key_alice, ciphertext)
send(r, 'byron', {"type": "communicate", "ciphertext": encrypt_message(session_key_byron, message).hex()})

j = receive(r)
ciphertext = bytes.fromhex(j["ciphertext"])
message = decrypt_message(session_key_byron, ciphertext)
send(r, 'alice', {"type": "communicate", "ciphertext": encrypt_message(session_key_alice, message).hex()})
